import { Injectable } from '@nestjs/common';
import { ParserService } from '../../parser';
import { ElasticsearchService } from '@nestjs/elasticsearch';
import { Focus } from '@kaiseratlas/parser';
import { ProductEntitiesService } from '../../shared/services/product-entities.service';

@Injectable()
export class FocusesService extends ProductEntitiesService(Focus) {
  constructor(
    protected readonly parserService: ParserService,
    protected readonly elasticsearchService: ElasticsearchService,
  ) {
    super(parserService, elasticsearchService);
  }

  protected readonly index = 'focuses';
  protected readonly mappings = {
    properties: {
      id: { type: 'text', copy_to: ['_all'] },
      name: { type: 'text', copy_to: ['_all'] },
      description: { type: 'text', copy_to: ['_all'] },
      product: {
        type: 'object',
      },
      _all: {
        type: 'text',
      },
    },
  };

  protected async transform(focus: Focus): Promise<any> {
    const [name, description] = await Promise.all([
      focus.getName(),
      focus.getDescription(),
    ]);
    return {
      id: focus.id,
      name: name?.value,
      description: description?.value,
    };
  }
}
